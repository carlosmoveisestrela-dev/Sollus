...
export default function CadastroCategoria() {
  ...
  async function buscarCategorias() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/categoria?page=${pagina}&limit=${tamanhoPagina}&busca=${encodeURIComponent(busca)}`
      )
  ...
  const selecionadas = categorias.filter(e => e.selecionada)
  ...
  function abrirModalEdicao(categoria) {
    ...
    setNomeEditando(categoria.categoria_nome)
    setModalAberto(true)
  }
  ...
  async function salvarEdicao() {
    ...
    try {
      const url = modoEdicao
        ? `${API_URL}/categoria/${categoriaEdicao.categoria_codigo}`
        : `${API_URL}/categoria`
  ...
  async function confirmarExclusaoLote() {
    ...
    try {
      await Promise.all(
        categoriasParaExcluir.map(categoria =>
          fetch(`${API_URL}/categoria/${categoria.categoria_codigo}`, {
            method: "DELETE"
          })
        )
      )
      message.success("Categoria(s) excluída(s) com sucesso!")
      fecharModalExcluir()
      fecharModalEdicao()
      buscarCategorias()
    } catch (error) {
        ...
        <table>
          ...
          <tbody>
            {carregando ? (
              ...
              <tr>
                <td colSpan={2} className="vazio">Nenhuma categoria cadastrada</td>
              </tr>
            ) : (
